...
func TestPrimitiveToString_Date(t *testing.T) {
	d := types.Date{Time: time.Date(2024, 6, 15, 0, 0, 0, 0, time.UTC)}
...
func TestDate_MarshalUnmarshalRoundtrip(t *testing.T) {
	d := types.Date{Time: time.Date(2024, 3, 15, 0, 0, 0, 0, time.UTC)}
...
func TestDate_Format(t *testing.T) {
	d := types.Date{Time: time.Date(2024, 12, 25, 0, 0, 0, 0, time.UTC)}
	assert.Equal(t, "2024-12-25", d.Format(types.DateFormat))
